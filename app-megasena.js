document.addEventListener('DOMContentLoaded', function() {
    ...
    const megaSenaWheelElement = document.getElementById('megaSenaWheel');
    
    // Dados da aplicação
    let megaSenaData = null;
    let delayedNumbers = [];
    let suggestedGames = [];
    ...
    function loadSampleData() {
        ...
        megaSenaData = {
            ...
            bottomPairs: [
                { numbers: [1, 60], frequency: 42 },
                { numbers: [3, 59], frequency: 43 },
                { numbers: [5, 57], frequency: 45 },
                { numbers: [8, 55], frequency: 46 },
                { numbers: [9, 56], frequency: 47 }
            ]
        };
        
        delayedNumbers = megaSenaData.delayedNumbers.map(item => item.number);
    }
    
    function updateUI() {
        if (!megaSenaData) return;
        
        // Atualizar informações básicas
        lastContestElement.textContent = megaSenaData.lastContest;
        contestDateElement.textContent = megaSenaData.contestDate;
        daysSinceElement.textContent = megaSenaData.daysSince;
        totalContestsElement.textContent = megaSenaData.totalContests;
        ...
        megaSenaData.topPairs.forEach(pair => {
        ...
        megaSenaData.bottomPairs.forEach(pair => {
            const li = document.createElement('li');
            li.innerHTML = `<span>${pair.numbers[0]} e ${pair.numbers[1]}</span> <strong>${pair.frequency}</strong>`;
    ...
    function generateSuggestedGames() {
        ...
        setTimeout(() => {
            ...
            // Gerar 6 jogos aleatórios com 6 números cada
            for (let i = 0; i < 6; i++) {
                const game = [];
                const allNumbers = Array.from({ length: 60 }, (_, i) => i + 1);
                
                // Incluir alguns números atrasados (1-3 por jogo)
                ...
                const otherNumbers = remainingNumbers
                    .sort(() => 0.5 - Math.random())
                    .slice(0, 6 - delayedInGame.length);
    ...
    function createWheel() {
        megaSenaWheelElement.innerHTML = '';
        ...
        for (let i = 1; i <= 60; i++) {
            ...
            megaSenaWheelElement.appendChild(numberEl);
        }
    }
